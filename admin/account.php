...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        // Update the account
        $stmt = $pdo->prepare('UPDATE users SET username = ?, password = ?, private_email = ?, activation_code = ?, rememberme = ?, role = ? WHERE id = ?');
        $password = $account['password'] != $_POST['password'] ? password_hash($_POST['password'], PASSWORD_DEFAULT) : $account['password'];
        $stmt->execute([ $_POST['username'], $password, $_POST['private_email'], $_POST['activation_code'], $_POST['rememberme'], $_POST['role'], $_GET['id'] ]);
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT IGNORE INTO users (username,password,email,activation_code,rememberme,role) VALUES (?,?,?,?,?,?)');
        $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $stmt->execute([ $_POST['username'], $password, $_POST['private_email'], $_POST['activation_code'], $_POST['rememberme'], $_POST['role'] ]);
        header('Location: index.php');
        exit;
    }
}
...
<?=template_admin_header($page . ' User account')?>

<h2><?=$page?> User account</h2>
...
<div class="content-block">
    <form action="" method="post" class="form responsive-width-100">
        ...
        <input type="text" id="private_email" name="private_email" placeholder="Email" value="<?=$account['private_email']?>" required>
