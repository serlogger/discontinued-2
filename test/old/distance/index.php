...
<body>
    ...
    if(!empty($distance_km) && !empty($latitude) && !empty($longitude)){ 
        ...
        $sql_distance = " ,(((acos(sin((".$latitude."*pi()/180)) * sin((`p`.`lat`*pi()/180))+cos((".$latitude."*pi()/180)) * cos((`p`.`lat`*pi()/180)) * cos(((".$longitude."-`p`.`lon`)*pi()/180))))*180/pi())*60*1.1515*1.609344) as distance "; 
    ...
    $sql = "SELECT p.*".$sql_distance." FROM services p $having ORDER BY $order_by"; 
    $query = $db->query($sql); 
    ?>
