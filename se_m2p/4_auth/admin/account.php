...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('UPDATE accounts SET username = ?, password = ?, email = ?, activation_code = ?, role = ?, registered = ?, last_seen = ? WHERE id = ?');
        $stmt->execute([ $_POST['username'], $password, $_POST['email'], $_POST['activation_code'], $_POST['role'], $_POST['registered'], $_POST['last_seen'], $_GET['id'] ]);
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare('INSERT IGNORE INTO accounts (username,password,email,activation_code,role,registered,last_seen) VALUES (?,?,?,?,?,?,?,?)');
        $stmt->execute([ $_POST['username'], $password, $_POST['email'], $_POST['activation_code'], $_POST['role'], $_POST['registered'], $_POST['last_seen'] ]);
